...
public class ReturnE2ETest {
    ...
    public void should_create_return_for_existing_order() {
        ...
        ReturnOrder returnOrder = returnClient.createReturn(returnToCreate, jwtToken);

        // Verify return
        assertThat(returnOrder).isNotNull();
        assertThat(returnOrder.returnId()).isNotNull().isNotEmpty();
        assertThat(returnOrder.orderId()).isEqualTo(firstOrder.orderId());
        assertThat(returnOrder.itemsToReturn()).hasSize(1);
        assertThat(returnOrder.status()).isNotNull();
        assertThat(returnOrder.createdAt()).isNotNull();

        System.out.println("✓ Return created successfully");
        System.out.println("Return ID: " + returnOrder.returnId());
        System.out.println("Status: " + returnOrder.status());
    }
    ...
    public void should_get_specific_return_by_id() {
        ...
        ReturnOrder createdReturn = returnClient.createReturn(returnToCreate, jwtToken);
        ...
        assertThat(fetchedReturn.itemsToReturn()).hasSize(1);
